...
const Home = () => {
  ...
  const videoCache = {
    ...
    setPageCache: (page: number, data: any) => {
      try {
        ...
        localStorage.setItem('ydx-home-max-page', String(Math.max(page, parseInt(localStorage.getItem('ydx-home-max-page') || '0'))));
  ...
  const fetchHomePageVideos = async (page: number = currentPage, isBackgroundFetch: boolean = false) => {
    try {
      ...
      if (cachedPage) {
        ...
        console.log(`Page ${page} fetch from cache took ${performance.now() - startTime}ms`);
        return;
      }
      ...
      if (!response.result || !response.result.videos || response.result.videos.length === 0) {
      ...
      console.log(`Page ${page} fetch from API took ${performance.now() - startTime}ms`);
    } catch (error) {
      // If the new endpoint fails, fall back to the original two-call approach
      console.error('Error with combined endpoint, falling back to original approach:', error);
      if (!isBackgroundFetch) {
  ...
  const parseHomePageData = useCallback((combinedData: any) => {
    ...
    if (!youtubeData || !youtubeData.items || youtubeData.items.length === 0) {
      ...
      const existingIds = new Set(
        videosData.map(video => video.key).filter(key => typeof key === 'string')
      );
      ...
      for (let i = 0; i < youtubeData.items.length; i += 1) {
        ...
        const thumbnailMedium = item.snippet.thumbnails.medium || item.snippet.thumbnails.default;
